...
此方法源于的题目：找出数组中出现次数超过一半的数。常规解法1:哈希表，统计每个元素出现的次数，找到大于一半的。时间和空间复杂度都是O(N). 常规解法2:排序，排序完之后，中间的那个数，必定是出现超过一半的数。时间复杂度O(logn)空间复杂度O(1).
...
* 当n1!=x：抵消的所有数字，众数x的数量为一半或0个。
利用此特性，每轮假设发生票数和=0都可以缩小剩余数组区间。当遍历完成时，最后一轮假设的数字即为众数。

算法流程:
1. 初始化：票数统计votes=0， 众数x；
2. 循环：遍历数组nums中的每个数字num。当票数votes等于0，则假设当前数字num是众数；当num=x时，票数votes自增1。当num!=x时，票数votes自减1；最后返回x即可。

拓展：由于题目说明给定的数组总是存在多数元素，因此本题不用考虑数组不存在众数的情况。若考虑，需要加入一个“验证环节”，遍历数组nums统计x的数量。
* 若x的数量超过数组长度一半，则返回x。
* 否则，返回未找到众数。
时间和空间复杂度不变，仍为O(N)和O(1)。

**约瑟夫环**

概念来源：N个人组成循环队列，每次把第K个人踢出去，然后下一个人重置报数，问最终谁可以留下来。

分析：这个问题乍一看，好像可以直接用循环队列解决。实际上，循环队列每次查找都需要遍历K个元素，会直接超时。

进一步推导过程，以N等于8，K等于3的情况来看整个过程的流转，如图所示：

![约瑟夫环1](/blog/2021/约瑟夫环1.png)

这个图展示了思考问题的流程，但是从结果上看，可能还是一头雾水。别急，我们接下来再看另一个图：

![约瑟夫环2](/blog/2021/约瑟夫环2.png)

已知最终留下来的人的下标为6，直接看第一次流转的情况。F(8,3)第一次淘汰的是C，然后从D继续，第二次F(7,3)淘汰的是F。我们从F(7,3)往回看，就相当于把C先补回来，然后序列就变成了DEFGHABC。这个时候，我们再把所有数右移3，有的索引就超过了最大数，对队列取模，就直接恢复了原队列。这意味着什么？意味着循环队列的规律就是，可以通过最后留下来的状态，逐步往前推导，得到最初的队列。而最后留下来的人的下标，一定是0，即F(1,3)=0，而F(2,3) = (F(1,3)+3) % 2。一般表达式是：F(n,k) = (F(n-1,k) + k) % n。最终即可算出初始队列中，哪个下标的元素会留下来。
```
func lastRemaining(n int, m int) int {
    pos := 0
    //超过两个人的情况
    //i就是队列人数的情况，每次加1表示不停地把人补回来
    for i := 2; i <= n; i++ {
        pos = (pos+m)%i
    }
    return pos
...
一般情况考察的是入度和出度的知识，如各种可达计算。

**关于GO的一些知识**

1、解题的时候，碰到了一个问题，就是在对数组进行操作的时候，发现append会改变数组里面原有的一些值。从而对里面进行了一些了解，发现go的数组有一个机制，核心数据结构里面有一个len和cap的字段，len指的就是数组实际元素的长度，而cap指的是数组的容量。当容量足够的时候，底层不会生成新的结构，当容量不足，则立马会进行翻倍扩容，生成新的空间，旧的结构等待被垃圾回收。所以，就有了为什么数组一定要在知道长度的情况下，先声明容量。
...
nums = nums[1:]
//剔除尾部元素
nums = nums[:len(nums)-1]   //也就是说[]的边界是含头不含尾
```
