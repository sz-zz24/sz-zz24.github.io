...
```
接下来，我们需要在配置文件中指名站点所使用的主题。打开config.toml直接编辑或者使用echo 'theme = "yourtheme"' >> config.toml命令。配置文件中还有其他的可配置内容，这些我们暂时不去管他。
Hugo的配置文件和文章中引用图片都是从static作为根目录的。写文章的时候可以直接从static目录引用。
...
自此运行hugo命令后生成的网页文件将保存在/docs子目录下。将所有文档push到Github的master branch，进入Github对应repository的Settings标签菜单，在GitHub Pages选项的Source栏选择master branch /docs folder:见前面域名配置的图。等待片刻即可访问域名看到之前用Hugo生成的网页了。
